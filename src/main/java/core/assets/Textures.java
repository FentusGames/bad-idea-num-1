...
public class Textures {
    ...
    public void loadFrom(Path folderPath) {
        ...
        try (Stream<Path> stream = Files.walk(folderPath)) {
            Map<String, List<Path>> groupedFiles = stream.filter(Files::isRegularFile)
                .filter(path -> path.toString().toLowerCase().endsWith(".png"))
                .collect(Collectors.groupingBy(path -> folderPath.relativize(path.getParent()).toString().toLowerCase().replace("\\", "_")));
